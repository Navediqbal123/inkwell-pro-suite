...
const HistoryPanel = ({ isOpen, onClose, history, onClearHistory, onRemoveItem }: HistoryPanelProps) => {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 flex items-start justify-center pt-20 md:pt-28"
      onClick={onClose}
    >
      {/* Backdrop */}
...
// This component now only renders the panel, no floating button
const HistorySection = ({ history, onClearHistory, onRemoveItem }: HistorySectionProps) => {
  const [isOpen, setIsOpen] = useState(false);

  // Listen for custom event to open history panel from user menu
  useEffect(() => {
    const handleOpenHistory = () => {
      setIsOpen(true);
    };

    window.addEventListener('openHistoryPanel', handleOpenHistory);
    return () => window.removeEventListener('openHistoryPanel', handleOpenHistory);
  }, []);
